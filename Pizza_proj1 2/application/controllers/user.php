...
class User extends CI_Controller{
     ...
     public function process(){
       ...
       if($this->input->post("quantity")){
           ...
           $insertArray=array();
               ...
               $insertArray['pd_id']=$this->input->post("pd");
    ...
    public function processOrder(){
       ...
       if($processFlag==1 && $updateTax==1){
           ...
           {
               $this->session->set_userdata('Error',"<strong>WARNING :</strong>Enter All Required Details '*'   ");
    ...
    public function ajaxPriceDetails(){
        if($this->input->post("pd")){
            $pd =   $this->input->post("pd");
            //$pd=2;
            $getPrice   =   $this->user_model->getPrizzaDetails($pd);
            echo number_format($getPrice->pd_price,2);
        }
    }
